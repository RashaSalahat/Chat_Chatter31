...
class _SplashScreenState extends State<SplashScreen> {
  late final StreamSubscription<firebase.User?> listener; // this give us the user
  ...
  Future<void> _handleAuthenticatedState() async {
    ...
    listener = auth.authStateChanges().listen((user) async {
      if (user != null) {
        //to know if we are authenticated
        // get Stream user token
        // you call the function after you got it
        final callable =
            FirebaseFunctions.instance.httpsCallable('getStreamUserToken');//stream token

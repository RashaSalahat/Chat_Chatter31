...
// });
// here we'll add the cloud functions
const StreamChat = require('stream-chat').StreamChat; //authenticate the stream api , generate tokens
const functions = require("firebase-functions");
const admin = require("firebase-admin");

admin.initializeApp();
// from env variables
//instance to create our stream chat
...
exports.getStreamUserToken = functions.https.onCall((data, context) => {
  ...
  if (!context.auth) {
    // Throwing an HttpsError so that the client gets the error details.(if auth is null)
    throw new functions.https.HttpsError('failed-precondition', 'The function must be called ' +
      'while authenticated.');
  } else {
    try {
      // we use the firebase uid as id
      return serverClient.createToken(context.auth.uid);
    } catch (err) {
